...
- **진행 중 연속 기록** — 대시보드에서 선수·팀·기간·현재 값을 확인
- **연속 기록 센터** — 진행 중 기록과 최근 종료 기록을 시즌·선수·팀·유형별로 조회
- 마일스톤 기준은 `milestones.csv`로 관리하며, 설정 탭에서 앱 내 편집 가능

### 기록 달성 예측

- 통산·시즌 마일스톤 달성 가능성 사전 예측
- 시즌 전체 페이스와 사용 가능한 최근 경기 구간을 구분하고, 현재값·목표값·예상 추가치의 계산 근거를 표시
...
- 선택 선수의 최근 이벤트, 진행 중 연속 기록, 다음 마일스톤을 한 패널에서 요약
...
> **수정된 박스스코어:** 정렬상 최신 경기는 수동 재가져오기 시 원본·마일스톤·연속 기록을 한 트랜잭션으로 교체합니다. 과거 경기와 같은 날짜의 복수 경기는 시점별 누적 기록을 안전하게 재생하는 기능이 추가될 때까지 변경 전에 거부됩니다.

### 기타

- **대시보드** — 최근 마일스톤, 임박 예측, 진행 중 연속 기록, DB 요약, 빠른 이동
- **달성 기록** — 이벤트 유형·중요도·자동/수동 필터, 유형·출처 열, 선수 상세 이동
- **레이팅 편집** — 현재 필터 대상의 변경 전후 미리보기·확인·백업 후 `mod_*_rosters.txt` 저장
