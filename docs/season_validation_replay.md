...
OOTP 27 `messages.dat`는 현재 확인된 바이너리 날짜 테이블을 직접 해석한다. 재생 워크플로는
뉴스 텍스트 파일과 같은 세이브 트리에서 `messages.dat`를 찾고, `messageN.txt`의 숫자 ID와
레코드 ID가 일치하는 경우 해당 날짜를 사용한다. 확인된 포맷과 제한은
[`messages_dat_format.md`](messages_dat_format.md)에 정리되어 있다.

파일 수정 시각은 경기 날짜로 추측하지 않는다. 알 수 없는 OOTP 버전, 손상된 레코드,
또는 날짜 테이블이 없는 환경에서는 기존처럼 JSON 또는 CSV 날짜 맵을 전달할 수 있다.
정확한 날짜가 필요한 이적·FA·연장계약·부상은 날짜가 없으면 기록하지 않고
`missing_date_count`에 남긴다.
